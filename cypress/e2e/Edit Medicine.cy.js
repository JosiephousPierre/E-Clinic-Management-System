describe('Edit functionality', () => {
    beforeEach(() => {
        ...
        cy.get('#drugName').clear().type('test');
        cy.get('#brandName').clear().type('test');
        cy.get('#quantity').clear().type('10');

        // Click on the date button
        cy.get('#expiration').should('be.visible').clear().click();
    ...
    it('Canceling adding Medicine', () => {
        ...
        // Wait for the alert and assert its content
        cy.on('window:alert', (message) => {
            expect(message).to.equal('Editing medicine has been canceled.');
    });
